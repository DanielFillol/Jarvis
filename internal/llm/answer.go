...
func (c *Client) answerWithModel(question, threadHistory, slackCtx, jiraCtx, model string) (string, error) {
	systemParts := []string{
		"Você é o Jarvis, assistente do Slack.",
		...
		"- Links de issue Jira: mencione a KEY diretamente, ex: TPTDR-123",
	...
	if strings.TrimSpace(c.JiraBaseURL) != "" {
		...
		systemParts = append(systemParts, "",
			...
			"- Formato: "+baseURL+"/browse/KEY (ex: "+baseURL+"/browse/GR-216)",
			"- NUNCA use outros domínios como musatribe.atlassian.net - use APENAS o base URL fornecido acima.")
	}
	...
	u.WriteString("\n\n")
	u.WriteString("Responda com tópicos curtos. Quando citar issue, inclua a KEY (ex: TPTDR-123).")
	msgs := []OpenAIMessage{
